...
test("slate quality weights exact source coverage",()=>{
  ...
  assert.equal(slateQualityScore({teams:full,starters:full,lineups:full,hitters:full,weather:full,parks:full,bullpens:full,opponentStrikeouts:full,calibrated:true}),100);
  assert.ok(slateQualityScore({teams:full,starters:{available:0,expected:10},lineups:full,hitters:full,weather:full,parks:full,bullpens:full,opponentStrikeouts:full,calibrated:true})<100);
});
