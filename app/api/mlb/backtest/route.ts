...
export async function GET(request:Request) {
  ...
  try {
    ...
    return Response.json({season,through,retrievedAt:new Date().toISOString(),source:"MLB Stats API",method:{name:"Date-batched expanding-window walk-forward",minimumPriorGames:10,featureRule:"Every game on a date uses one prior-day snapshot; results update state only after the date batch",marketDataIncluded:false},gamesIngested:games.length,...result},{headers:{"cache-control":"public, max-age=1800, stale-while-revalidate=3600"}});
