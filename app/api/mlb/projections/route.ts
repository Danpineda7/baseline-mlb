  ...
  try {
    ...
    const games = scheduledGames.map((game) => {
      ...
      return {
        ...
        quality:{teamRecords:Boolean(away&&home),starterStats:missingStarters===0,awayStarter:Boolean(awayStarter),homeStarter:Boolean(homeStarter),bullpen:Boolean(bullpenResponse.ok),opponentStrikeouts:Boolean(awayOpponent&&homeOpponent),rosterAvailability:transactionsResponse.ok,platoon:Boolean([...awayLineup,...homeLineup].length&&[...awayLineup,...homeLineup].every(hitter=>hitter.platoonAdjusted)),lineups:lineupsConfirmed,weather:conditions.temperature!=null,parkSample:parkSample.homeGames>=20&&parkSample.roadGames>=20,blockingReasons:[!away||!home?"Team season record missing":null,missingStarters?"One or more probable-starter stat lines missing":null].filter((reason):reason is string=>Boolean(reason))},
        uncertainty,
