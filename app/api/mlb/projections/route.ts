...
export async function GET(request: Request) {
  ...
  try {
    const [scheduleResponse, standingsResponse, contextResponse, bullpenResponse, teamHittingResponse, transactionsResponse] = await Promise.all([
    ...
    let firstInningShare=0.115,firstFiveShare=0.56,contextGames=0,calibrationModel:CalibrationModel|null=null,calibrationQualified=false,calibrationMethod="pending";const marketCalibrationModels:{totalOver85:CalibrationModel|null;firstFiveHome:CalibrationModel|null;nrfi:CalibrationModel|null}={totalOver85:null,firstFiveHome:null,nrfi:null},marketCalibrationMethods={totalOver85:"pending",firstFiveHome:"pending",nrfi:"pending"},marketCalibrationQualified={totalOver85:false,firstFiveHome:false,nrfi:false};const historicalGames:HistoricalGame[]=[],teamEnvironments=new Map<number,{homeGames:number;homeRuns:number;roadGames:number;roadRuns:number}>();
    if(contextResponse.ok){const context=await contextResponse.json() as ContextPayload;let allRuns=0,firstRuns=0,firstFiveRuns=0;for(const game of (context.dates??[]).flatMap(day=>day.games??[])){if(game.status?.abstractGameState!=="Final")continue;const innings=game.linescore?.innings??[];const awayScore=game.teams?.away?.score??0,homeScore=game.teams?.home?.score??0,total=awayScore+homeScore,awayId=game.teams?.away?.team?.id??0,homeId=game.teams?.home?.team?.id??0;if((game.gamePk??0)>0&&awayId>0&&homeId>0&&game.gameDate)historicalGames.push({id:game.gamePk??0,playedAt:game.gameDate,awayId,homeId,awayScore,homeScore,firstInningAway:innings[0]?.away?.runs,firstInningHome:innings[0]?.home?.runs,firstFiveAway:innings.length?innings.filter(inning=>(inning.num??0)<=5).reduce((sum,inning)=>sum+(inning.away?.runs??0),0):undefined,firstFiveHome:innings.length?innings.filter(inning=>(inning.num??0)<=5).reduce((sum,inning)=>sum+(inning.home?.runs??0),0):undefined});if(!innings.length||total<=0)continue;const homeEnvironment=teamEnvironments.get(homeId)??{homeGames:0,homeRuns:0,roadGames:0,roadRuns:0},awayEnvironment=teamEnvironments.get(awayId)??{homeGames:0,homeRuns:0,roadGames:0,roadRuns:0};teamEnvironments.set(homeId,{...homeEnvironment,homeGames:homeEnvironment.homeGames+1,homeRuns:homeEnvironment.homeRuns+total});teamEnvironments.set(awayId,{...awayEnvironment,roadGames:awayEnvironment.roadGames+1,roadRuns:awayEnvironment.roadRuns+total});allRuns+=total;firstRuns+=(innings[0]?.away?.runs??0)+(innings[0]?.home?.runs??0);firstFiveRuns+=innings.filter(inning=>(inning.num??0)<=5).reduce((sum,inning)=>sum+(inning.away?.runs??0)+(inning.home?.runs??0),0);contextGames+=1;}if(allRuns>0){firstInningShare=clamp(firstRuns/allRuns,0.08,0.16);firstFiveShare=clamp(firstFiveRuns/allRuns,0.45,0.68);}const validation=walkForwardBacktest(historicalGames,10);calibrationModel=validation.liveCalibration;calibrationQualified=Boolean(calibrationModel&&validation.calibratedMetrics?.verified);calibrationMethod=validation.calibratedMetrics?.selectedMethod??"pending";for(const key of ["totalOver85","firstFiveHome","nrfi"] as const){marketCalibrationModels[key]=validation.liveMarketCalibrations[key];marketCalibrationMethods[key]=validation.marketCalibratedMetrics[key]?.selectedMethod??"pending";marketCalibrationQualified[key]=Boolean(marketCalibrationModels[key]&&validation.marketCalibratedMetrics[key]?.verified);}}
    ...
    const games = scheduledGames.map((game) => {
      ...
      const rawDistribution = projectScore(awayRuns, homeRuns);
      ...
      const calibratedOver=calibratedDelta(baseDistribution.over,rawDistribution.over,marketCalibrationModels.totalOver85,marketCalibrationQualified.totalOver85,marketCalibrationMethods.totalOver85),distribution={...rawDistribution,over:calibratedOver,under:1-calibratedOver};
      ...
      return {
        ...
        quality:{teamRecords:Boolean(away&&home),starterStats:missingStarters===0,awayStarter:Boolean(awayStarter),homeStarter:Boolean(homeStarter),bullpen:Boolean(bullpenResponse.ok),opponentStrikeouts:Boolean(awayOpponent&&homeOpponent),rosterAvailability:transactionsResponse.ok,platoon:Boolean([...awayLineup,...homeLineup].length&&[...awayLineup,...homeLineup].every(hitter=>hitter.platoonAdjusted)),lineups:lineupsConfirmed,weather:conditions.temperature!=null,parkSample:parkSample.homeGames>=20&&parkSample.roadGames>=20,validatedMarkets:{moneyline:calibrationQualified,total:marketCalibrationQualified.totalOver85,firstFive:marketCalibrationQualified.firstFiveHome,firstInning:marketCalibrationQualified.nrfi,pitcherStrikeouts:false,hitterHits:false},blockingReasons:[!away||!home?"Team season record missing":null,missingStarters?"One or more probable-starter stat lines missing":null].filter((reason):reason is string=>Boolean(reason))},
        uncertainty,
