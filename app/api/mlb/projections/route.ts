import { bullpenFatigueAdjustment, clamp, empiricalParkFactor, fairAmerican, firstInningMarkets, hitterHitProjection, inningsToDecimal, projectPeriod, projectScore, starterRunAdjustment, strikeoutExpectation } from "@/lib/modeling";
...
export async function GET(request: Request) {
  ...
  try {
    const [scheduleResponse, standingsResponse, contextResponse, bullpenResponse] = await Promise.all([
    ...
    const games = scheduledGames.map((game) => {
      ...
      return {
        ...
        lineups:{away:lineup(game.lineups?.awayPlayers??[]),home:lineup(game.lineups?.homePlayers??[]),confirmed:lineupsConfirmed},
        quality:{teamRecords:Boolean(away&&home),starterStats:missingStarters===0,awayStarter:Boolean(awayStarter),homeStarter:Boolean(homeStarter),bullpen:Boolean(bullpenResponse.ok),lineups:lineupsConfirmed,weather:conditions.temperature!=null,parkSample:parkSample.homeGames>=20&&parkSample.roadGames>=20,blockingReasons:[!away||!home?"Team season record missing":null,missingStarters?"One or more probable-starter stat lines missing":null].filter((reason):reason is string=>Boolean(reason))},
        uncertainty,
