...
export async function syncOddsPapiDate(db:D1Like,key:string,date:string,games:GameRef[],limit=3){
  ...
  for(let fixtureIndex=0;fixtureIndex<queue.length;fixtureIndex++){
    ...
    const history=await get<{bookmakers:Record<string,{markets:Record<string,{outcomes:Record<string,{players:Record<string,Array<{createdAt:string;price:number;limit:number|null;active:boolean}>>}>}>}>}>("historical-odds",key,{fixtureId:fixture.fixtureId,bookmakers:"pinnacle,draftkings,fanduel"});const statements=[] as D1Statement[];
