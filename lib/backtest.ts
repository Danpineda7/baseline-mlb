...
export function walkForwardBacktest(games: HistoricalGame[], minimumPriorGames = 10) {
  ...
  // Final walk-forward team states, exposed so live projections can compute
  // their baseline with the exact estimator the calibration was fitted on.
  const teamRates=[...teams.entries()].map(([teamId,state])=>({teamId,games:state.games,scored:state.scored,allowed:state.allowed}));
  const liveLeagueAverage=leagueTeamGames>0?leagueRuns/leagueTeamGames:null;
  return { predictions, metrics:{count,accuracy,brier,logLoss,totalMae},marketMetrics,calibratedMetrics,marketCalibratedMetrics,calibration:buckets,liveCalibration,liveMarketCalibrations,teamRates,liveLeagueAverage };
}
