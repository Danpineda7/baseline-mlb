...
export function gradeBet(bet:SettlementInput,game:FinalGame){
  ...
  else if(bet.market==="awayK"||bet.market==="homeK"){if(bet.subjectId==null||bet.line==null)return null;const value=game.players[bet.subjectId]?.strikeOuts;result=value==null?"VOID":value===bet.line?"PUSH":value>bet.line?"WON":"LOST";}
  else if(bet.market==="hit"){if(bet.subjectId==null)return null;const value=game.players[bet.subjectId]?.hits;result=value==null?"VOID":value>=1?"WON":"LOST";}
